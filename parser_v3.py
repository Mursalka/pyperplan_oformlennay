...
class State(object):
    ...
    def apply(self, action):
        ...
        new_preds |= set(action.effects)
        new_preds -= set(action.preconditions)
        new_functions = dict()
...
def is_true(state, predicates):
    return all(p in state.predicates for p in predicates) and sum(['holding' in i[0] for i in state.predicates]) < 3
        ...
        if node not in closed:
            ...
            for successor in successors:
                if successor not in closed:
                    heapq.heappush(fringe, (heuristic(), successor))
...
def Astar_planner(problem, heuristic=h_g_heuristic, state0=None, goal=None):
    if heuristic is None:
        heuristic = null_heuristic
    if state0 is None:
        state0 = problem.init
